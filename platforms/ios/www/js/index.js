...
function getll(address){
    ...
    $.getJSON("http://maps.googleapis.com/maps/api/geocode/json", {"address":address, "sensor":true}).done(function(data){
        window.ll = [data["results"][0]["geometry"]["location"]["lat"], data["results"][0]["geometry"]["location"]["lng"]];
        ...
        $(document).ready(function(){
            ...
            $('#logi').on("tap",function(event){
                ...
                $("#dispSelect").change(function(event){
                        ...
                        $.post("http://app.d2dpro.com/update_disp.php", {"dispid":window.dispdata, "disp":val});
                });
                $('#dispInfo').on('pageshow', function(){
                    console.log("at dispInfo");
                    console.log(window.dispdata);
                    disp = window.dispositions[window.dispdata];
                    console.log(disp);
                    $("#di-name").val(disp[0]);
                    $("#di-addr").val(disp[1]);
                    ...
                    $('#mapdiv').gmap({'disableDefaultUI':true}).bind('init',function(event,map){
                        $.getJSON( 'http://app.d2dpro.com/get_locations.php',{"userID":window.userID}).done(function(data) {
                            ...
                            for (var dataiter=0; dataiter < data.length; dataiter++){
                                ...
                                var position = getll(data[dataiter][1][1]+","+data[dataiter][1][3]+","+data[dataiter][1][4]);
                                var ourdata=dataiter;
                                window.dispositions.push(data[dataiter][1]);
                                /*
                                ...
                                */
                                ...
                                console.log(data[dataiter][1][1]+","+data[dataiter][1][3]+","+data[dataiter][1][4]);
                                console.log(ourdata);
                                console.log("------");

                                var addmarkertoeval = "$('#mapdiv').gmap('addMarker', {'position': '"+position2+"', 'bounds':true}).click(function(){window.dispdata = '"+ourdata+"';$.mobile.changePage('#dispInfo', 'slide');});";
